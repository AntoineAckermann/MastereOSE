...
data_hydro = { "H1": ["hydro", 1, 0, 900, 1500, 90, 0, 0.31, 3000],
               "H2": ["hydro", 1, 0, 1400, 1200, 150, 0, 0.47, 3000] } # N, Pmin, Pmax, Cstart, Cbase, Cmwh, water_consumption, elec_consumption

data_thermal = pd.DataFrame.from_dict(data_thermal, orient='index', columns=["Type", "N", "Pmin", "Pmax", "Cstart", "Cbase", "Cmwh"])
data_hydro = pd.DataFrame.from_dict(data_hydro, orient='index', columns=["Type", "N", "Pmin", "Pmax", "Cstart", "Cbase", "Cmwh", "water_consumption", "elec_consumption"])
...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    m.Hydro.water_consumption = pyo.Param(m.Hydro.Plants, initialize=df_hydro["water_consumption"].to_dict(),
                                          mutable=False)
    m.Hydro.elec_consumption = pyo.Param(m.Hydro.Plants, initialize=df_hydro["elec_consumption"].to_dict(),
                                         mutable=False)
    ...
    def define_demand(m, t):
        return m.total_demand[t] == m.Demand[t]
