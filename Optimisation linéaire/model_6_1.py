...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    def n_plants_thermal(b, p, t):
        return b.n[p, t] == sum(b.on[p,k,t] for k in b.Units[p])
    ...
    def no_pump_while_generating(b, t):
        M_in = sum(m.Hydro.water_consumption[p, 4]*m.Hours[t] for p in b.Plants for t in m.T ) # le max de pompage est égal au max possible de consommation d'eau sur la journée, i.e. les deux centrales hydro tournent à leur palier max
