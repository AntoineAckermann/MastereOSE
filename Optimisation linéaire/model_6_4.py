...
new_demand_2h = [16,14,16,30,28,25,24,33,41,32,26,22]

periods_discretized_2h = {f"{t}h-{t+1}h": [new_demand_2h[t//2]*1e3, 1] for t in range(2*len(new_demand_2h))}

data_periods = pd.DataFrame.from_dict(periods_discretized_2h, orient='index', columns=["Demand", "Hours"])
...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    def power_reserve(m, t):
        ...
        return margin_thermal + margin_hydro >= margin * m.total_demand[
            t]  # la somme des centrales qui produisent déjà doit pouvoir encaisser une hausse de 15% de la demande
