...
data_thermal = {"A": [12, 850, 2000, 1.5], # N, Pmin, Pmax, Cmwh
         "B": [10, 1250, 1750, 1.38],
         "C": [5, 1500, 4000, 2.75]}
...
data_thermal = pd.DataFrame.from_dict(data_thermal, orient='index', columns=["N", "Pmin", "Pmax", "Cmwh"])
data_periods = pd.DataFrame.from_dict(periods, orient='index', columns=["Demand", "Hours"])


def build_model(df_plants=data_thermal, df_periods=periods):
    ...
    def total_cost():
        return sum(model.prod[plant, period]*model.Hours[period]*model.Cmwh[plant] for plant in model.Plants for period in model.Periods)
