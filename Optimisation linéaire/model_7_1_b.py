...
periods_discretized = {t: [periods_temp[period_old][0], 1] for period_old in periods_temp.keys() for t in
                       range(period_old[0], period_old[1])}
print(periods_discretized)
...
data_thermal = pd.DataFrame.from_dict(data_thermal, orient='index',
                                      columns=["Type", "N", "Pmin", "Pmax", "Cstart", "Cbase", "Cmwh"])
data_hydro = pd.DataFrame.from_dict(data_hydro).set_index(["centrale", "level"])


def build_model_7_1(df_thermal, df_hydro, df_periods, name="Daily planning", n_last=None):
    # 0. MODELE GLOBAL
    ...
    def maintenance_thermal(b, k, t):
        return b.on["A", k+1, t] <= 0
    ...
    m.Thermal.maintenance = pyo.Constraint([0, 1], list(m.T)[:6]+list(m.T)[18:], rule=maintenance_thermal)
...
non_cyc_model = build_model_7_1(data_thermal, data_hydro, data_periods,
                                name="Daily non-cyclical planning w/ hydro")  # il faut d'abord faire tourner le modèle avec planification non-cyclique pour obtenir les dernières valeurs pour les intégrer dans le modèle cyclique
optimizer.solve(non_cyc_model)

last_t = non_cyc_model.T.last()
n_last_dict = {p: pyo.value(b.n[p, last_t]) for b in non_cyc_model.Blocks for p in b.Plants}

model = build_model_7_1(data_thermal, data_hydro, data_periods, n_last=n_last_dict,
                        name="Daily cyclical planning w/ hydro")
