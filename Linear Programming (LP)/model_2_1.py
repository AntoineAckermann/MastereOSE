...
data_thermal = {"A": [12, 850, 2000, 2000, 1000, 2.0], 
...
def build_model(df_plants, df_periods):
    ...
    def total_cost():
        return sum( ( model.Cbase[plant] * model.n[plant, period] + model.Cmwh[plant] * ( model.prod[plant, period] - model.Pmin[plant] * model.n[plant, period] ) ) * model.Hours[period] for plant in model.Plants for period in model.Periods)
