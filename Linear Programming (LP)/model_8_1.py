...
data_thermal = {"A": ["thermal", 12, 850, 2000, 2000, 1000, 2.0, 400],  # Type, N, Pmin, Pmax, Cstart, Cbase, Cmwh, ramp_limit
                "B": ["thermal", 10, 1250, 1750, 1000, 2600, 1.3, 600],
                "C": ["thermal", 5, 1500, 4000, 500, 3000, 3, 800]}
...
data_thermal = pd.DataFrame.from_dict(data_thermal, orient='index',
                                      columns=["Type", "N", "Pmin", "Pmax", "Cstart", "Cbase", "Cmwh", "RampMax"])
...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    def ramp_limit(b, p, k, t):
        return b.prod[p, k, t+1] - b.prod[p, k, t] <= b.RampMax[p]
    ...
    m.Thermal.ramp_limit = pyo.Constraint(m.Thermal.PU, list(m.T)[:-1], rule=ramp_limit)
