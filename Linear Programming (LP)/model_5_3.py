...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    def flow_in_rule(b):
        return sum(b.flow_in[t] for t in m.T) - sum(b.flow_out[p, t] for p in b.Plants for t in
                                                    m.T) == 0  # niveau d'eau à l'équilibre sur la journée  # niveau d'eau à l'équilibre sur la journée
