...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    m.Hydro.Elec_pump = pyo.Param(initialize=elec_consumption)
    ...
    def power_reserve(m, t):
        ...
        margin_hydro = sum(m.Hydro.Pmax[p,l]*m.Hydro.y[p, l, t] - m.Hydro.prod[p,t] for (p, l) in m.Hydro.PlantLevels) 
    ...
    def cost_thermal():
        ...
        Cmwh_thermal = sum( m.Thermal.Cmwh[p] * ( m.Thermal.prod[p, t] - m.Thermal.Pmin[p] * m.Thermal.n[p, t] ) * m.Hours[t] for p in m.Thermal.Plants for t in m.T )
