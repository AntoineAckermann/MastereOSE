...
def build_model(df_thermal, df_hydro, df_periods, opt, cyc=True):
    ...
    m.Thermal.s = pyo.Var(m.Thermal.PU, m.T, within=pyo.Binary) # centrale démarée au temps t
    m.Thermal.e = pyo.Var(m.Thermal.PU, m.T, within=pyo.Binary) # centrale éteinte au temps t
    m.Thermal.n = pyo.Var(m.Thermal.Plants, m.T, within=pyo.NonNegativeIntegers) # nombre de centrales de la techno en fonctionnement à t
    m.Thermal.n_start_up = pyo.Var(m.Thermal.Plants, m.T, within=pyo.NonNegativeIntegers) # nombres de centrales démarrées au temps t
    ...
    m.Hydro.water_consumption = pyo.Param(m.Hydro.PlantLevels, initialize = df_hydro["water_consumption"].to_dict(), mutable=False)
    ...
    m.Hydro.n_start_up = pyo.Var(m.Hydro.Plants, m.T, within=pyo.NonNegativeIntegers) # nombres de centrales nouvellement allumées à chaque pas de temps    
    m.Hydro.y = pyo.Var(m.Hydro.PlantLevels, m.T, within=pyo.Binary) # y[p, l, t] = 1 si la centrale hydro p tourne au niveau l à la période t, 0 sinon
    ...
    def define_demand(m, t):
        return m.total_demand[t] == m.Demand[t] + m.Hydro.flow_in[t] * m.Hydro.Elec_pump / m.Hours[t]   # la consommation pour remonter le niveau d'eau se rajoute à la demande totale
    ...
    def power_reserve(m, t):
        ...
        margin_thermal = sum(m.Thermal.Pmax[p]*m.Thermal.n[p,t] - sum(m.Thermal.prod[p, k, t] for k in m.Thermal.Units[p]) for p in m.Thermal.Plants)
    ...
    def unit_started(b, p, k, t):
        if t == m.T.first():
            return b.s[p, k, t] >= b.on[p, k, t]
        else:
            return b.s[p, k, t] >= b.on[p, k, t] - b.on[p, k, m.T.prev(t)]

    def unit_ended(b, p, k, t):
        if t == m.T.last():
            # pas d'arrêt "utile" au dernier pas (ou on met une contrainte triviale)
            return b.e[p, k, t] >= 0
        else:
            return b.e[p, k, t] >= b.on[p, k, t] - b.on[p, k, m.T.next(t)]
    
    def minimum_uptime(b, p, k, t):
        return sum(b.on[p, k, tau] for tau in range(t, t+L)) >= L * b.s[p, k, t]
    
    def minimum_downtime(b, p, k, t):
        # t doit être tel que t+L <= dernier indice, donc on le restreint dans l’appel de Constraint
        return sum(1 - b.on[p, k, tau] for tau in range(t+1, t+L+1)) >= L * b.e[p, k, t]
    ...
    m.Thermal.unit_started = pyo.Constraint(m.Thermal.PU, m.T, rule=unit_started)
    m.Thermal.unit_ended = pyo.Constraint(m.Thermal.PU, list(m.T)[:-1], rule=unit_ended)
    m.Thermal.min_uptime = pyo.Constraint(m.Thermal.PU, list(m.T)[:-(L-1)], rule=minimum_uptime)
    m.Thermal.min_downtime  = pyo.Constraint(m.Thermal.PU, list(m.T)[:-L], rule=minimum_downtime)
    ...
    def flow_out_rule(b, p, t):
        return b.flow_out[p, t] == sum( b.water_consumption[p, l] * b.y[p, l, t] * m.Hours[t] for (pp, l) in b.PlantLevels if pp==p )  # si la centrale hydro fonctionne, elle consomme de l'eau
    
    def flow_in_rule(b):
        return sum( b.flow_in[t] for t in m.T ) == sum( b.flow_out[p, t] for p in b.Plants for t in m.T )  # niveau d'eau à l'équilibre sur la journée 
    ...
    def no_pump_while_generating(b, t):
        ...
        return b.flow_in[t] <= M_in * (1 - sum(b.y[p, l, t] for p in b.Plants for (pp,l) in b.PlantLevels if p==pp)) # 
    ...
    def cost_thermal():
        ...
        Cmwh_thermal = sum( m.Thermal.Cmwh[p] * ( m.Thermal.prod[p,k,t] - m.Thermal.Pmin[p] * m.Thermal.on[p, k, t] ) * m.Hours[t] for p in m.Thermal.Plants for k in m.Thermal.Units[p] for t in m.T )
    ...
    def cost_hydro():
        ...
        Cbase_hydro = sum( m.Hydro.Cbase[p, l] * m.Hydro.y[p, l, t] * m.Hours[t] for (p, l) in m.Hydro.PlantLevels for t in m.T )
